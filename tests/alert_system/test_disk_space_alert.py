...
def make_statvfs(f_frsize=1, f_blocks=0, f_bfree=0, f_bavail=0):
    return MagicMock(
        f_frsize=f_frsize, f_blocks=f_blocks, f_bfree=f_bfree, f_bavail=f_bavail
    )
...
class TestGetDiskUsage:
    ...
    def test_propagates_oserror_for_missing_path(self):
        with (
            patch.object(
                dsa.os, "statvfs", side_effect=OSError("No such file or directory")
            ),
...
class TestGetLowDiskSpaceAlerts:
    ...
    def test_missing_or_unmounted_path_is_flagged(self):
        with patch.object(
            dsa, "get_disk_usage", side_effect=OSError("No such file or directory")
        ):
            alerts = dsa.get_low_disk_space_alerts(["/not/mounted"])
        assert len(alerts) == 1
        assert alerts[0]["path"] == "/not/mounted"
        assert (
            "not found" in alerts[0]["alert_message"]
            or "not mounted" in alerts[0]["alert_message"]
        )
...
class TestMainDiskSpaceAlert:
    def test_no_slack_call_when_no_alerts(self):
        with (
            patch.object(dsa, "get_low_disk_space_alerts", return_value=[]),
    ...
    def test_sends_to_each_webhook_when_alerts_present(self):
        ...
        with (
            patch.object(dsa, "get_low_disk_space_alerts", return_value=fake_alerts),
            patch.object(
                dsa.su, "get_webhook_list", return_value=["hook1", "hook2"]
            ) as mock_get_webhooks,
            patch.object(dsa.su, "send_slack_notification") as mock_send,
        ...
        mock_get_webhooks.assert_called_once_with(
            dsa.slack_configuration_dictionary, dsa.lab
        )
