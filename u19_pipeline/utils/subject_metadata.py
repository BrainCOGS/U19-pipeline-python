...
def fetch_slack_handles_for_lab_managers_by_subject(
    ...
    if isinstance(subjects, str):
        subjects = [subjects]
    sql_formatted_names = (
        ", ".join([f"'{name}'" for name in subjects]) if subjects else "''"
    )
    expanded_animal = (
        (subject.Subject & f"subject_fullname in ({sql_formatted_names})")
        * lab.User()
        * lab.UserLab()
    )
    ...
    lab_managers = (lab.LabManager() & f"lab in ({sql_formatted_labs})").proj(
        user_id="lab_manager"
    ) * lab.User()
