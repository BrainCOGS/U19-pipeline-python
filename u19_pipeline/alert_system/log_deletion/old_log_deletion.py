...
def main_old_log_deletion():
    ...
    files_df["exctract_date"] = files_df["filename"].str.extract(
        r"(\d{8," + str(8) + r"})"
    )
    files_df["exctract_date"] = pd.to_datetime(
        files_df["exctract_date"], format="%Y%m%d"
    )
    files_df["before_ref_date"] = files_df["exctract_date"] <= pd.Timestamp(
        reference_date
    )
    ...
    for file_path in files_for_deletion:
        if (
            file_path.is_file()
        ):  # Check if it's actually a file before attempting to delete
            file_path.unlink()
