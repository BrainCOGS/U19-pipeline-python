...
def main():
    ...
    subject_rig_df = bm.BehaviorMetrics.get_zscore_metric_session_df(
        subject_rig_df, "num_trials", "session_location"
    )
    ...
    subject_rig_df["abs_z_score_num_trials"] = np.abs(
        subject_rig_df["z_score_num_trials"]
    )
    num_trials_alert_df = subject_rig_df.loc[
        subject_rig_df["abs_z_score_num_trials"] >= zscore_alert, :
    ].copy(deep=True)

    # Get sign of zscpre (only group subjects with same deviation)
    num_trials_alert_df["sign_zscore"] = np.sign(
        num_trials_alert_df["z_score_num_trials"]
    )
    num_trials_alert_df["sign_zscore"] = num_trials_alert_df["sign_zscore"].astype(int)

    # Count how many subjects were biassed by rig
    num_trials_alert_location_df = num_trials_alert_df.groupby(
        ["session_location", "sign_zscore"]
    ).agg(
        {
    ...
    num_trials_alert_location_df.columns = (
        num_trials_alert_location_df.columns.droplevel()
    )
    ...
    num_trials_alert_location_df2 = num_trials_alert_location_df.groupby(
        ["session_location"]
    ).agg({"session_location": [("num_bias_sides", "size")]})
    num_trials_alert_location_df2.columns = (
        num_trials_alert_location_df2.columns.droplevel()
    )
    ...
    num_trials_alert_location_df = num_trials_alert_location_df.loc[
        num_trials_alert_location_df["num_subjects"] > 1, :
    ]
    ...
    num_trials_alert_location_df["alert_message"] = (
        "Multiple subjects had abnormal number of trials in this rig"
    )
