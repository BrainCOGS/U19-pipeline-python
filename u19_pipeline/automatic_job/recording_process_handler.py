...
class RecProcessHandler:
    ...
    def pipeline_handler_main():
        ...
        for i in range(df_all_process_job.shape[0]):
            ...
            function_status_process = getattr(
                RecProcessHandler, next_status_series["ProcessFunction"]
            )

            # Trigger process, if success update recording process record
            try:
                status, update_dict = function_status_process(
                    rec_process_series, next_status_series
                )
                ...
                if status == config.status_update_idx["NEXT_STATUS"]:
                    ...
                    RecProcessHandler.update_status_pipeline(
                        key, next_status, field_update, value_update
                    )
                ...
                if status != config.status_update_idx["NO_CHANGE"]:
                    if status == config.status_update_idx["ERROR_STATUS"]:
                        ...
                        if len(update_dict["error_info"]["error_message"]) > 255:
                            ...
                            update_dict["error_info"]["error_message"] = update_dict[
                                "error_info"
                            ]["error_message"][-255:]

                        if (
                            isinstance(
                                update_dict["error_info"]["error_exception"], str
                            )
                            and len(update_dict["error_info"]["error_exception"]) > 4095
                        ):
                            print("Cropping error error_exception")
                            update_dict["error_info"]["error_exception"] = update_dict[
                                "error_info"
                            ]["error_exception"][-4095:]
        ...
        if rec_series["program_selection_params"]["process_cluster"] == "tiger":
            if status_series["Key"] == "RAW_FILE_TRANSFER_REQUEST":
                ...
                transfer_request = ft.globus_transfer_to_tiger(
                    job_id, raw_rel_path, modality
                )
            elif status_series["Key"] == "PROC_FILE_TRANSFER_REQUEST":
                # ALS, which recording directory for processed file
                transfer_request = ft.globus_transfer_to_pni(
                    job_id, proc_rel_path, modality
                )
            ...
            if transfer_request["status"] == config.system_process["SUCCESS"]:
                ...
                update_value_dict["error_info"]["error_message"] = transfer_request[
                    "error_info"
                ]
        ...
        if rec_series["program_selection_params"]["process_cluster"] == "tiger":
            ...
            if transfer_request["status"] == config.system_process["COMPLETED"]:
                ...
                update_value_dict["error_info"]["error_message"] = (
                    "An error occured during globus transfer"
                )
    ...
    def slurm_job_queue(rec_series, status_series):
        ...
        if status == config.system_process["SUCCESS"]:
            ...
            status_update = config.status_update_idx["ERROR_STATUS"]
            update_value_dict["error_info"]["error_message"] = (
                "Error while generating/transfering parameter file"
            )
            return (status_update, update_value_dict)
        ...
        if (
            ...
            and rec_series["recording_modality"] == "electrophysiology"
        ):
            recording_key = (
                recording_process.Processing.proj(
                    "recording_id", insertion_number="fragment_number"
                )
            ...
            chanmap_file_local_path = pathlib.Path(
                config.chanmap_files_filepath, chanmap_filename
            ).as_posix()
            raw_directory_for_chanmap = rec_series["recording_process_pre_path"]

            spikeglx_meta_filepath = ephys_pipeline.get_spikeglx_meta_filepath(
                recording_key
            )
            # Chanmap mat file generation
            MetaToCoords(
                spikeglx_meta_filepath, 1, destFullPath=chanmap_file_local_path
            )
            status = paramfilelib.generate_chanmap_file(
                rec_series["job_id"], rec_series["program_selection_params"]
            )
            if status != config.system_process["SUCCESS"]:
                status_update = config.status_update_idx["ERROR_STATUS"]
                update_value_dict["error_info"]["error_message"] = (
                    "Error while generating/transfering channelmap file"
                )
                return (status_update, update_value_dict)
        ...
        if rec_series["program_selection_params"]["local_or_cluster"] == "cluster":
            ...
            if status == config.system_process["SUCCESS"]:
                ...
                status_update = config.status_update_idx["ERROR_STATUS"]
                update_value_dict["error_info"]["error_message"] = (
                    "Error while generating/transfering slurm file"
                )
                return (status_update, update_value_dict)
            ...
            if status == config.system_process["SUCCESS"]:
                ...
                status_update = config.status_update_idx["ERROR_STATUS"]
                update_value_dict["error_info"]["error_message"] = (
                    "Error while queuing job in SLURM"
                )
                return (status_update, update_value_dict)
    ...
    def slurm_job_check(rec_series, status_series):
        ...
        if rec_series["program_selection_params"]["local_or_cluster"] == "cluster":
            ...
            if (
                program_selection_params["process_cluster"] == "spock"
                and is_this_spock()
            ):
                local_user = True

            ssh_user = ft.cluster_vars[program_selection_params["process_cluster"]][
                "user"
            ]
            ssh_host = ft.cluster_vars[program_selection_params["process_cluster"]][
                "hostname"
            ]
            slurm_jobid = str(rec_series["slurm_id"])

            status_update, message = slurmlib.check_slurm_job(
                ssh_user, ssh_host, slurm_jobid, local_user=local_user
            )
    ...
    def get_program_selection_params(modality):
        ...
        this_modality_program_selection_params_dict = (
            this_modality_program_selection_params.to_dict("records")
        )
        this_modality_program_selection_params_dict

        # Get two columns, (recording_modality & "packed" program_selection_params)
        this_modality_program_selection_params = (
            this_modality_program_selection_params.loc[:, "recording_modality"]
            .to_frame()
            .copy()
        )
        this_modality_program_selection_params["program_selection_params"] = (
            this_modality_program_selection_params_dict
        )
    ...
    def get_active_process_jobs(active=True):
        ...
        if active:
            ...
            status_query += " and status_processing_id < " + str(
                config.JOB_STATUS_PROCESSED
            )
        else:
            status_query = "status_processing_id = " + str(config.JOB_STATUS_ERROR_ID)
            status_query += " or status_processing_id = " + str(
                config.JOB_STATUS_PROCESSED
            )

        jobs_active = (
            recording.Recording.proj(
                "recording_modality", "location", "recording_directory"
            )
        ...
        if df_process_jobs.shape[0] > 0:
            ...
            df_process_jobs["query_key"] = df_process_jobs.loc[:, key_list].to_dict(
                orient="records"
            )
            ...
            for this_modality in all_modalities:
                this_mod_df = df_process_jobs.loc[
                    df_process_jobs["recording_modality"] == this_modality, :
                ].copy()
                these_process_keys = this_mod_df["query_key"].to_list()

                this_mod_program_selection_params = (
                    RecProcessHandler.get_program_selection_params(this_modality)
                )

                if this_modality == "electrophysiology":
                    params_df = RecProcessHandler.get_ephys_params_jobs(
                        these_process_keys
                    )

                if this_modality == "imaging":
                    params_df = RecProcessHandler.get_imaging_params_jobs(
                        these_process_keys
                    )
            ...
            for this_mod_df in all_mods_df[1:]:
                df_process_jobs = pd.concat(
                    [df_process_jobs, this_mod_df], ignore_index=True
                )
    ...
    def get_ephys_params_jobs(rec_process_keys):
        ...
        params_df = pd.DataFrame(
            (
                ephys_pipeline.ephys_element.ClusteringParamSet.proj(
        ...
        preparams_df = pd.DataFrame(
            (
                ...
                * utility.smart_dj_join(
                    ...
                    ephys_pipeline.ephys_element.PreClusterParamSet.proj(
                        "precluster_method", "params"
                    ),
                )
                * recording_process.Processing.EphysParams.proj(
    ...
    def update_status_pipeline(
        recording_process_key_dict, status, update_field=None, update_value=None
    ):
        """
    ...
    def update_job_id_log(job_id, current_status, next_status, error_info_dict):
        ...
        if (
            error_info_dict["error_message"] is not None
            and len(error_info_dict["error_message"]) >= 256
        ):
            error_info_dict["error_message"] = error_info_dict["error_message"][:255]

        if (
            error_info_dict["error_exception"] is not None
            and len(error_info_dict["error_exception"]) >= 4096
        ):
            error_info_dict["error_exception"] = error_info_dict["error_exception"][
                :4095
            ]
    ...
    def check_job_process_deletion():
        ...
        df_inactive_jobs["local_or_cluster"] = df_inactive_jobs[
            "program_selection_params"
        ].map(lambda x: x["local_or_cluster"])
        df_inactive_jobs["process_cluster"] = df_inactive_jobs[
            "program_selection_params"
        ].map(lambda x: x["process_cluster"])
        ...
        if df_not_cluster.shape[0] > 0:
            ...
            df_not_cluster_post_processed = df_not_cluster_post_processed.reset_index(
                drop=True
            )
        ...
        df_inactive_jobs["days_from_last_update"] = (
            now - df_inactive_jobs["max_date"]
        ).dt.days
        ...
        df_inactive_jobs["processed_dir"] = df_inactive_jobs["processed_dir"].astype(
            str
        )
        ...
        for i in range(df_post_error.shape[0]):
            RecProcessHandler.update_status_pipeline(
                df_post_error.loc[i, "query_key"], config.JOB_STATUS_ERROR_DELETED
            )
            RecProcessHandler.update_job_id_log(
